...
```
## 说明
+  这里显示引用 `lua-resy-core` 是因为 `lua-resty-core` 把 `lua-nginx-module` 中的部分 API 使用 FFI 方式实现，FFI 方式实现的代码可以被 JIT 追踪和优化，性能更高
...
+ dns_resolver 数组元素有两个来源，首先是 `config-default.yaml` 配置文件中 dns_resolver 的配置，如果该配置文件中未配置 dns_resolver ，则从系统配置文件 `/etc/resolv.conf` 中获得系统配置的域名解析服务器信息。

+ 文首的 `config-default.yaml` 中 dns_resolver 配置被注释掉，所以是从 `/etc/resolv.conf` 中获得域名解析服务器信息。
`cat /etc/resolv.conf`
```
...
-- 若当前操作系统是 Linux，设置 PCRE 引擎的 jit 栈大小为200k。详见注解2
...
2. 若文件存在，则 `uid` 为文件内容，文件为空，则 `uid` 为空字符串
